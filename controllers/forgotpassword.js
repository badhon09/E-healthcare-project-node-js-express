...
router.post('/', (req, res)=>{
	...
	userModel.getByEmail(email, function(results) {
        ...
        req.session.user_name = results.fullname;
...
router.get('/send-mail', function (req, res){
    ...
    var mailOptions = {
       ...
       // text: 'To reset your password please enter the following code in the field: '+verification_code,
        html: `<p><h4>Password Reset Request</h4> <ul><li>Name:${ req.session.user_name}
        </li><li>email:${req.session.user_mail}</li>
        <li><b>Verification Code:${ verification_code}</b></li></ul></p>`,
    };
        ...
        if(req.body.password==req.body.cpassword){
            ...
            userModel.updatePass(user, function (callback){
                if(callback == true){
                    ...
                    res.redirect('/login');
                   // req.flash('mesg' , 'Password Updated,Log in with New Password');
                }
                else{
